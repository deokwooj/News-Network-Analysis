...
#### Output
   - 인용문(문장)의 연결망 그래프
     (K=1, K=2, K=3, .. 등이 표현됨, 같은 클러스터 내의 q 는 같은 색상으로)
   ...
   - 엑셀 분석 파일
     ...
     - 인용문의 거리 계산에 따른 정리 sheet
       - 거리가 <= 1인경우, <= 2인 경우, <=3인경우, <=4인경우
         (즉 엑셀의 시트에 K=1, K=2, ... 등이 표현됨)

     - 엑셀에 degree, 즉 neighbor 의 갯수를 카운트해서 넣어야 함.
       - degree 가 높은 순으로 searching 하는 알고리즘을 작성하고, (K=n) 에 따라 서칭한 결과를 출력해볼 것
